...
class MenuPage(webapp2.RequestHandler):
  def get(self):
    ...
    template1 = jinja_env.get_template('main.html')
...
class ResponsiveWeb(Handler):
  def get(self):
    self.render("responsive_web_design_fundimentals.html")

class IntroToHTMLandCSS(Handler):
  def get(self):
    self.render("Intro_to_html_and_css.html")



app = webapp2.WSGIApplication([("/", MenuPage),
                             ("/sign", CommentWall),
                             ("/responsive_web_design_fundimentals",ResponsiveWeb),
                             ("/Intro_to_html_and_css",IntroToHTMLandCSS)], debug=True)
